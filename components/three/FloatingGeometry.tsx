...
const vertexShader = `
  ...
  void main() {
    ...
    float noise = snoise(position * 0.8 + uTime * 0.3) * 0.15;
    vDisplacement = noise;
...
const fragmentShader = `
  ...
  void main() {
    ...
    // Color gradient driven by time and displacement
    float colorMix = sin(uTime * 0.5 + vDisplacement * 4.0) * 0.5 + 0.5;
    vec3 baseColor = mix(uColor1, uColor2, colorMix);

    // Combine fresnel glow with base color
    vec3 finalColor = mix(baseColor * 0.6, baseColor * 1.8, fresnel);
    finalColor += fresnel * uColor2 * 0.5;
